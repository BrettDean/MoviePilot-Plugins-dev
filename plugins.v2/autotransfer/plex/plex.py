...
class Plex:
    ...
    def refresh_library_by_items_modified(self, items: List[schemas.RefreshMediaItem]) -> Optional[bool]:
        """
        ...
        if "" in result_dict:
            ...
            for path, lib_key in result_dict.items():
                ...
                self._plex.query(f'/library/sections/{lib_key}/refresh?path={quote_plus(str(Path(path)))}')
